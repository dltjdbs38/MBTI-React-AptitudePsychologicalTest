...
export default function TestPage() {
  ...
  for (let i = 0; i < saveData.length; i = i + 5) {
    question5.push(saveData.slice(i, i + 5));
  }
  ...
  if (question5 && question5.length > 0) {
    console.log("question5 : 5개씩 6묶음 성공");
  ...
  function NextPage() {
    if (pageCount < question5.length - 1 && pageNum < question5.length) {
      setPageCount(pageCount + 1);
      setPageNum(pageNum + 1);
      pagenum = pageNum;
      history.push(`/test/${pagenum}`);
    } else if (pageCount === 5 && pageNum === 6) {
      setPageCount(question5.length - 1);
      setPageNum(question5.length);
      pagenum = pageNum;
      history.push(`/test/${pagenum}`);
    } else {
      setPageNum(1);
